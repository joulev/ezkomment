...
In the front-end, other than the obvious page components and other utilities components (labelled yellow below) required for each page, we make use of several React contexts (labelled green below) for state management. These contexts will handle data fetching, hence making it no longer necessary to fetch data on every route change, or rerender everything when the user navigates to a different page. That improves not only performance but user experience as well.

We also make use of the React error boundary to handle errors. It will help us log all client-side errors that we forget to catch or do not know about.
...
![How the comment section is rendered](/images/orbital/others/activity-diagram-get.png)

![What happens when a new comment is posted](/images/orbital/others/activity-diagram-post.png)
...
In any types of programming, duplicating code is a bad idea, and this application is no exception. Whenever something looks like it will be used more than once, it is a requirement that it is moved to a separate utility module.

One notable example is the `<A>` React component, used to replace the `<a>` HTML tag. It is a simple component: if the URL is external it adds extra attributes to the `<a>` tag to open the link in a new tab, otherwise it uses Next.js' router to handle the link. Something simple like that _can_ be written directly in the pages without having to extract to a separate component. However, since it will be used in a lot of places, we decided to extract it regardless to an utility component where all the logic related to it is maintained.
...
7. When writing the customised template, the page may become unresponsive if a lot of JavaScript libraries are added to the template (fixed in v1.0.0-rc.1.3).

8. The customise page has weird scrolling behaviour in full screen mode (fixed in v1.0.0-rc.1.3).

9. The customise page should have an auto-save mechanism to avoid data loss on accidental tab/window close or page refresh, which may be very frustrating for users (fixed in v1.0.0-rc.1.3).

10. The `POST` method of the REST API fails access control checks (fixed in v1.0.0-rc.1.3).

11. Many links in the app was pointing to `https://google.com` as placeholder, but now they should be updated with useful URL (fixed in v1.0.0-rc.1.3).
