...
> The Milestone 2 README is written on top of [Milestone 1 README](/orbital/ms1-readme). Probably you do not want to read the whole thing again, so look out to headings with an [updated] label: they are modified after Milestone 1.
...
[Lighthouse](https://developer.chrome.com/docs/lighthouse/overview) is a popular tool for evaluating the performance of a website. We conduct Lighthouse score check frequently and also use Vercel analytics to monitor, from there we can make suitable improvement to improve the whole application.
...
<BlogImage
  src="/images/markdown/lighthouse-desktop-20220624.png"
  caption="Lighthouse scores for desktop, test done at 2:53pm 24 June 2022 (SGT)"
/>

### Security |updated|

Authentication workflow is mainly done by the Firebase client library. With it being one of the most popular libraries of its type, with about 1.3 millions downloads per week, I think it is safe to say this step is as secure as we can possibly get. Especially compared to self-made authentication pattern which may fall to many well-known attacks, such as [Cross-Site Request Forgery](https://en.wikipedia.org/wiki/Cross-site_request_forgery) attacks if cookie-based authentication is used.
...
The contribution process is described in slightly more details in the [CONTRIBUTING file](https://github.com/joulev/ezkomment/blob/main/CONTRIBUTING.md).
...
However, in the first half of June, we had quite a few problems with miscommunication between the two members, thereby leading to a significant delay of the project:
