...
The back-end is currently written with [Express.js](https://expressjs.com). Currently, we are planning to deploy our backend API using [Firebase Hosting](https://firebase.google.com/docs/hosting) - however, we are still considering other options for suitable hosting cost and deployment process.
...
- `/users`: The route to get, update and delete user information
  - `/users/get`: Getting user basic information
  - `/users/update`: Updating user basic information
  - `/users/delete`: Deleting ALL user information
- `/sites`: The route to get, create, update and delete sites
- `/pages`: The route to get, create, update and delete pages

The Route Structure will be further improved as we progress. Nested routes will be used to show the hierarchy relation between the entites.
