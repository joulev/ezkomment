...
Even if we do allow everyone to reply to any comment, it is unlikely that we will only support one level deep (so Reddit-like threads are not supported).
...
Depending on whether we ultimately choose Express or Next.js API routes for the backend, this URI may become

```
https://ezkomment.joulev.dev/api/comments/{siteName}/{pageId}
```
...
The back-end is currently planned to be written in [Express.js](https://expressjs.com), and [@VietAnh1010](https://github.com/VietAnh1010) is making some progress in it. We decided to use TypeScript for the entire project for easier management, and talking of web servers in Node.js, Express is pretty much the single best choice. However, since Next.js also offers API routes to act as a back-end, we will only decide whether to use Express or Next.js for the back-end when we implement the app, after considering relevant security configurations, deployment processes and hosting costs.
...
We have not had a clear plan of how all back-end API routes are. We will implement them incrementally as the app features are added.
...
The logo was drawn with Figma. SVG versions can be found in [the repository](https://github.com/joulev/ezkomment/tree/main/packages/client/public/images).
...
In pull request [#66](https://github.com/joulev/ezkomment/pull/66), we have added the necessary
[SEO](https://en.wikipedia.org/wiki/Search_engine_optimization) tags to every page that need them,
and also set up a process to
...
Although we have not enable the site to be crawled for search engines as of now, the tags are now
effective when sharing the pages over social media.
