...
class ReplaceableReact {
  ...
  value: string | React.ReactNodeArray;
...
function TableTime({ time }: { time: string }) {
  return (
    <>
      {time
        .split(",")
...
function TableContent({ content }: { content: string }) {
  return <>{new ReplaceableReact(content).replaceAll()}</>;
}
function TableRemarks({ remarks }: { remarks: string }) {
  if (remarks[0] === "*") {
    return (
      <ul className="!ml-6">
        {remarks.split("\n").map((str, i, arr) => (
...
export default function OrbitalLogJoulevPage() {
  return (
    <BlogLayout
      ...
      <div className="flex flex-col gap-18">
        <div className="max-w-prose mx-auto">
          ...
          <p>
            ...
            day it commences (12 March 2022 &ndash; the date of the first commit) until 1 August
            2022. Since all activities from 12 March to 7 May are not properly logged as they are
            done, the time spent on studying and playing with new technologies, as well as the time
            to make small fixes and improvements have to be merged to one entry. All other entries
            also have to be derived from the commit history.
          </p>
          ...
          <p className="mb-0">
            ...
            <abbr title="face-to-face">F2F</abbr> is not possible, and we choose instant messaging
            for all communications. Therefore during the summer, we have no direct meetings, whether
